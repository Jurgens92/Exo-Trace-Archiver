...
The easiest way to deploy Exo-Trace-Archiver is using the automated install script on an Ubuntu 24.04 server. This sets up everything including PostgreSQL, Nginx, HTTPS, and systemd services.

### Prerequisites

- Ubuntu 24.04 server (fresh or existing)
- Root/sudo access
- (For HTTPS) A domain name pointing to your server's public IP
- (For HTTPS) Ports 80 and 443 open in your firewall

### Quick Install (HTTP)

```bash
# Download and run the install script
curl -sL https://raw.githubusercontent.com/Jurgens92/Exo-Trace-Archiver/main/install.sh -o install.sh
...
# Run with your domain and HTTPS enabled
PUBLIC_DOMAIN=traces.yourdomain.com ENABLE_HTTPS=true ADMIN_EMAIL=admin@yourdomain.com sudo -E bash install.sh
```

### Install with IP Address (No HTTPS)

```bash
PUBLIC_DOMAIN=203.0.113.50 sudo -E bash install.sh
```

### What the Install Script Does

1. Installs system dependencies (Python 3.12, Node.js 20, PostgreSQL, Nginx)
2. Clones the repository to `/opt/exo-trace-archiver`
3. Creates a PostgreSQL database with a random secure password
4. Sets up Python virtual environment and installs dependencies
5. Configures Django backend with production `.env` file
6. Builds the React frontend
7. Configures Nginx as a reverse proxy with security headers
8. (If HTTPS) Obtains a Let's Encrypt SSL certificate with auto-renewal
9. Creates systemd services for the backend and scheduler
10. Creates a default admin account

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `PUBLIC_DOMAIN` | No | Your domain name or public IP (defaults to detected private IP) |
| `ENABLE_HTTPS` | No | Set to `true` for Let's Encrypt SSL (requires a domain, not an IP) |
| `ADMIN_EMAIL` | No | Email for Let's Encrypt renewal notifications |
...
All credentials (including the generated database password) are saved to `/opt/exo-trace-archiver/installation_credentials.txt` (readable only by root).

### Post-Install: Configure Microsoft 365

After installation, configure your Microsoft 365 tenant:

1. Log in to the web interface
2. Go to **Tenants** > **Add Tenant**
3. Enter your Azure AD Tenant ID and Client ID
4. Upload your certificate (`.pfx` file) or enter client secret
5. Test the connection

Or edit the `.env` file directly:
```bash
sudo nano /opt/exo-trace-archiver/backend/.env
sudo systemctl restart exo-trace-backend
```

### Services

The install script creates three systemd services:

| Service | Description | Command |
|---------|-------------|---------|
| `exo-trace-backend` | Django API via Gunicorn | `systemctl restart exo-trace-backend` |
| `exo-trace-scheduler` | Daily automated trace pulls | `systemctl restart exo-trace-scheduler` |
| `nginx` | Reverse proxy + frontend | `systemctl restart nginx` |

### Viewing Logs

```bash
# Backend API logs
journalctl -u exo-trace-backend -f

# Scheduler logs
journalctl -u exo-trace-scheduler -f

# Nginx logs
tail -f /var/log/nginx/error.log

# Application logs
tail -f /opt/exo-trace-archiver/backend/logs/exo_trace_archiver.log
```

### Updating

```bash
curl -sL https://raw.githubusercontent.com/Jurgens92/Exo-Trace-Archiver/main/update.sh -o update.sh
sudo bash update.sh
```

The update script will:
- Create a full backup (files + database) before updating
- Pull the latest code from GitHub
- Update backend and frontend dependencies
- Run database migrations and rebuild the frontend
- Restart all services and verify they're healthy
- Generate a restore script for this specific backup

Backups are stored in `/opt/exo-trace-archiver-backups/`. To restore:
```bash
sudo bash /opt/exo-trace-archiver-backups/exo-trace-archiver_backup_TIMESTAMP/restore_this_backup.sh
```

### HTTPS / SSL Certificate Details

When `ENABLE_HTTPS=true` is set, the install script:

1. Installs certbot with the Nginx plugin
2. Starts Nginx on port 80 (required for ACME challenge)
3. Requests a certificate from Let's Encrypt
4. Configures Nginx to serve on port 443 with SSL and redirect HTTP to HTTPS
5. Enables Django security settings (`SECURE_SSL_REDIRECT`, `CSRF_TRUSTED_ORIGINS`, secure cookies)
6. Sets up automatic certificate renewal via certbot timer (runs twice daily)

Certificates auto-renew before expiry. To check renewal status:
```bash
...
> **Note:** If you used `install.sh` for production deployment, the scheduler is already running as a systemd service (`exo-trace-scheduler`). The options below are for development or manual setups.
