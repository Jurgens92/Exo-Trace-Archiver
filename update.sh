...
################################################################################
# Exo-Trace-Archiver Update Script for Ubuntu 24.04
#
...
BACKUP_DIR="/opt/exo-trace-archiver-backups"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_PATH="$BACKUP_DIR/exo-trace-archiver_backup_$TIMESTAMP"

# Check if Exo-Trace-Archiver is installed
if [ ! -d "$INSTALL_DIR" ]; then
    log_error "Exo-Trace-Archiver is not installed at $INSTALL_DIR"
...
log_info "Starting Exo-Trace-Archiver update..."
echo "========================================"
...
# Check if SQLite database file exists first
if [ -f "$INSTALL_DIR/backend/db.sqlite3" ]; then
    ...
    source <(grep -E '^(DATABASE_URL|DB_NAME|DB_USER|DB_PASSWORD)=' "$INSTALL_DIR/backend/.env" | sed 's/^/export /')
...
log_info "Stopping Exo-Trace-Archiver services..."
systemctl stop exo-trace-scheduler || log_warning "Scheduler service not running"
systemctl stop exo-trace-backend || log_warning "Backend service not running"
...
if ! systemctl is-active --quiet exo-trace-scheduler; then
    log_error "Scheduler service failed to start!"
...
if [ "$BACKEND_STATUS" = "running" ] && [ "$SCHEDULER_STATUS" = "running" ] && [ "$NGINX_STATUS" = "running" ]; then
    log_success "Exo-Trace-Archiver update completed successfully!"
else
...
echo "  - Check status:        systemctl status exo-trace-backend"
echo ""
...
if [ "$BACKEND_STATUS" != "running" ] || [ "$SCHEDULER_STATUS" != "running" ] || [ "$NGINX_STATUS" != "running" ]; then
    ...
    echo ""
    echo "  Or manually:"
    echo "  1. Stop services: systemctl stop exo-trace-scheduler exo-trace-backend"
    echo "  2. Restore files: rm -rf $INSTALL_DIR && cp -r $BACKUP_PATH $INSTALL_DIR"
    echo "  3. Restart services: systemctl start exo-trace-backend exo-trace-scheduler && systemctl restart nginx"
    echo ""
else
...
echo "Restoring Exo-Trace-Archiver from backup..."
echo "Backup: $BACKUP_PATH"
echo ""

# Stop services
systemctl stop exo-trace-scheduler || true
systemctl stop exo-trace-backend || true
...
if [ -f "$BACKUP_PATH/database_backup.sql" ]; then
    ...
    # Try to extract credentials from DATABASE_URL
    if [ -f "$INSTALL_DIR/backend/.env" ]; then
        DATABASE_URL=$(grep -E '^DATABASE_URL=' "$INSTALL_DIR/backend/.env" | cut -d'=' -f2-)
        if [ ! -z "$DATABASE_URL" ]; then
            DB_USER=$(echo "$DATABASE_URL" | sed -n 's|.*://\([^:]*\):.*|\1|p')
            DB_PASSWORD=$(echo "$DATABASE_URL" | sed -n 's|.*://[^:]*:\([^@]*\)@.*|\1|p')
            DB_NAME=$(echo "$DATABASE_URL" | sed -n 's|.*/\([^?]*\).*|\1|p')
...
echo "Exo-Trace-Archiver has been restored to the backup from: $(basename $BACKUP_PATH)"
RESTORE_SCRIPT
